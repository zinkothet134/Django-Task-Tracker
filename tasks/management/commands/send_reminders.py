...
class Command(BaseCommand):
    help = "Send task reminders 1 day before due date"
    ...
    def handle(self, *args, **kwargs):
        ...
        tasks = Task.objects.filter(
            due_date=tomorrow,
            status__in=[Task.TODO, Task.IN_PROGRESS]  # ✅ ignore done/blocked
        )
...
""",
                    ...
                    self.stderr.write(
                        self.style.ERROR(f"Failed for task {task.id}: {e}")
                    )

        self.stdout.write(
            self.style.SUCCESS(f"Reminders sent successfully: {sent_count}")
        )